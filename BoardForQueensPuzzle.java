...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
		...
		if (Math.abs(filesWithQueens[lastRankFilled] - filesWithQueens[rank]) == Math.abs(lastRankFilled - rank))
		return true;	
	}
    ...
    public boolean accept() {
         ...
         for (int rank = 0; rank < filesWithQueens.length - 1; rank++) {
		if (filesWithQueens[filesWithQueens.length - 1] == filesWithQueens[rank])
		return false;
